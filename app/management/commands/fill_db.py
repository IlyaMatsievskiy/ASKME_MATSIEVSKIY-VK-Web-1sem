...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        print(f'Генерация профилей и юзеров заняла {(time.time() - time_now) / 60} минут')
        tags = self.create_tags(ratio)
        print(f'Генерация тегов заняла {(time.time() - time_now) / 60} минут')
        questions = self.create_questions(ratio * 10, tags, profiles)
        print(f'Генерация вопросов заняла {(time.time() - time_now) / 60} минут')
        answers = self.create_answers(ratio * 100, questions, profiles)
        print(f'Генерация ответов заняла {(time.time() - time_now) / 60} минут')
        self.create_question_likes(ratio * 100, questions, profiles)
        print(f'Генерация лайков вопросов заняла {(time.time() - time_now) / 60} минут')
        self.create_answer_likes(ratio * 100, answers, profiles)
        print(f'Генерация лайков ответов заняла {(time.time() - time_now) / 60} минут')
